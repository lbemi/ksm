...
const PersistentVolumeClaimPage: FC = () => {
  ...
  const columns: TableProps<PersistentVolumeClaim>["columns"] = [
    {
      title: "名称",
    ...
    },
    {
      ...
      dataIndex: "spec",
      key: "resources",
      render: (spec) => <div>{spec?.resources?.requests?.["storage"]}</div>,
    },
    {
      title: "状态",
      dataIndex: "status",
    ...
    },
    {
      ...
      dataIndex: "action",
      width: 100,
      render: (_, record: PersistentVolumeClaim) => (
            ...
            menu={{
              items: [
                ...
                { key: "edit", label: "编辑", icon: <EditOutlined /> },
                {
                  ...
                  icon: <DeleteOutlined />,
                  danger: true,
                  onClick: () => handleDeletePersistentVolumeClaim(record),
                },
                { type: "divider" },
  ...
  const handleDeletePersistentVolumeClaim = (
      ...
      onOk: async () => {
        try {
          await kubernetes_request(
            "DELETE",
            `/api/v1/namespaces/${persistentVolumeClaim.metadata?.namespace}/persistentvolumeclaims/${persistentVolumeClaim.metadata?.name}`
          );
          message.success(
            `PersistentVolumeClaim ${persistentVolumeClaim.metadata?.name} 删除成功`
          );
  ...
  const list_persistentVolumeClaims = () => {
    setLoading(true);
    let url: string;
    if (namespace === "all") {
    ...
    kubernetes_request<Array<PersistentVolumeClaim>>("GET", url)
      .then((res) => {
        setPersistentVolumeClaims(res);
      })
      .catch((err) => {
        console.log("err: ", err);
      });
